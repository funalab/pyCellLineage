...
def Analysis_all(path, instMode, thr=None, atpMax=None, genMax=None):
    ...
    cellDFWP = measurePhenotypes(paths['matFilePath'], paths['segImgsPath'],
                                 paths['rawImgsPath'])
    #parent of path
    ...
    if mode['lineage']['save']:
        ...
        create2DLineage(cellDFWP,
                        attr='intensity',
                        attrMax=atpMax,
                        ylim=genMax,
                        savePath=savePathLin)
    ...
    if mode['hmmPrep']['normal']['mean']:
        ...
        cellDFWP = hmm_prep(cellDFWP,
                            save_dir=saveDir_Indi,
                            lname="low",
                            hname="high")
        hmmCellDF = cellDFWP
    ...
    if mode['oscillation']['prep']:
        ...
        write_ATPChange(cellDFWP,
                        saveDir_ATPIndi,
                        changedCSVpath=paths['changeCSV'])
...
def run():
    ...
    if mode['hmmPrep']['totalATP']['mean']:
        ...
        thr = sum(totalDF['intensity']) / len(totalDF['intensity'])
    elif mode['hmmPrep']['95ATP']['both'] or mode['hmmPrep']['95ATP'][
            'control']:
        totalDF = Total_ATP(instMode=windows)
        thr = nineFivePercentile(totalDF)
    elif mode['hmmPrep']['totalATP']['gmmPoor']:
        ...
        for num in sampleNum:
            if samples[cond][num]:
                print("Doing " + cond + " " + num + "\n\t Path:" +
                      samplePath[cond][num])
                Analysis_all(samplePath[cond][num],
                             windows,
                             thr=thr,
                             atpMax=atpMax,
                             genMax=genMax)
